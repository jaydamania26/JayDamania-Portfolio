...
import UIEventBus from '../UI/EventBus'; // Ensure path is correct
...
export default class MonitorScreen extends EventEmitter {
    ...
    setupBlockerEvents() {
        // Toggle Blocker based on Camera State
    ...
    initializeScreenEvents() {
        // ... (Keep existing mouse events for legacy iframe support)
        document.addEventListener('mousemove', (event) => {
            // @ts-ignore
            if (event.target.id === 'computer-screen') event.inComputer = true;
            // @ts-ignore
    ...
    createIframe() {
        ...
        // ... (Iframe onload logic - Copy existing logic here if needed) ...
        
        iframe.src = 'https://inner-site-green.vercel.app/';
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.has('dev')) iframe.src = 'http://localhost:8080/';
        ...
        blocker.id = 'monitor-blocker';
        
        // Touch/Click to Zoom
        const triggerZoom = (e: Event) => {
            // e.preventDefault(); // Optional, depending on scrolling needs
            this.camera.trigger('enterMonitor');
        };
        blocker.addEventListener('click', triggerZoom);
        blocker.addEventListener('touchstart', triggerZoom, {passive: false});
    ...
    createCssPlane(element: HTMLElement) {
        ...
        // IMPORTANT: HITBOX NAME FOR RAYCASTER
        mesh.name = 'computer-screen-hitbox'; 

        this.scene.add(mesh);
    }
    
    // ... (Rest of functions: createTextureLayers, etc. - copy from previous file)
    createTextureLayers() {
        ...
        const scaleFactor = 4;
        const layers = {
            smudge: { texture: textures.monitorSmudgeTexture, blending: THREE.AdditiveBlending, opacity: 0.12, offset: 24 },
            innerShadow: { texture: textures.monitorShadowTexture, blending: THREE.NormalBlending, opacity: 1, offset: 5 },
            video: { texture: this.videoTextures['video-1'], blending: THREE.AdditiveBlending, opacity: 0.5, offset: 10 },
            video2: { texture: this.videoTextures['video-2'], blending: THREE.AdditiveBlending, opacity: 0.1, offset: 15 },
        };
        let maxOffset = -1;
    ...
    createEnclosingPlanes(maxOffset: number) {
        const planes = {
            left: { size: new THREE.Vector2(maxOffset, this.screenSize.height), position: this.offsetPosition(this.position, new THREE.Vector3(-this.screenSize.width / 2, 0, maxOffset / 2)), rotation: new THREE.Euler(0, 90 * THREE.MathUtils.DEG2RAD, 0) },
            right: { size: new THREE.Vector2(maxOffset, this.screenSize.height), position: this.offsetPosition(this.position, new THREE.Vector3(this.screenSize.width / 2, 0, maxOffset / 2)), rotation: new THREE.Euler(0, 90 * THREE.MathUtils.DEG2RAD, 0) },
            top: { size: new THREE.Vector2(this.screenSize.width, maxOffset), position: this.offsetPosition(this.position, new THREE.Vector3(0, this.screenSize.height / 2, maxOffset / 2)), rotation: new THREE.Euler(90 * THREE.MathUtils.DEG2RAD, 0, 0) },
            bottom: { size: new THREE.Vector2(this.screenSize.width, maxOffset), position: this.offsetPosition(this.position, new THREE.Vector3(0, -this.screenSize.height / 2, maxOffset / 2)), rotation: new THREE.Euler(90 * THREE.MathUtils.DEG2RAD, 0, 0) },
        };
